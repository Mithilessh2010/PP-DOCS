...
This will cause the robot to turn from the `startPose`'s heading to the `endPose`'s heading while it is following the path. 
...
For example: 
``` java
Path path = line(startPose, endPose).linear(startPose, endPose, 0.8);
// or use radians for your interpolation
Path path = line(startPose, endPose).linear(Math.toRadians(90), Math.toRadians(180), 0.8);
```
...
``` java
Path path = line(startPose, endPose).constant(startPose);
// or use radians for your interpolation
Path path = line(startPose, endPose).constant(Math.toRadians(90));
```
