...
```
* **Explanation:** This creates a piecewise interpolation where the heading follows the path tangent until `t = 0.5` (about halfway through the path), then linearly interpolates between the start and end headings until `t = 1`. `t` is the normalized path parameter (start = 0, end = 1).
