...
In `Constants.java`, add an instance of `OTOSConfig`. Make sure to
replace the name with the actual name.

```java title="Constants.java"
public static OTOSConfig localizerConstants = new OTOSConfig(
  c -> {
    c.name.set("otos")
    c.linearUnit(DistanceUnit.INCH)
    c.offset.set(new Pose(3, 5));
    c.linearScalar.set(0.5);
    c.angularScalar.set(0.3)
  }
);
```

Then, add `new OTOSLocalizer` to `create`:
...
Add the linear scalar to your `OTOSConfig` by doing the following.
...
Add the linear scalar to your `OTOSConfig` (or modify the value in the quickstart)
...
Add the angular scalar to your `OTOSConfig` by doing the following.
