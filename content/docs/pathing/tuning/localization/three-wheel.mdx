...
In `Constants.java`, add an instance of `ThreeWheelConfig`. Make sure to replace the hardware map names
...
Then, add `.threeWheelLocalizer` to `create`:
...
To reverse an encoder, add one of the following to `ThreeWheelConfig`:
...
Add the multiplier to your `ThreeWheelConfig` by adding the following.
...
Lastly, add the multiplier to `ThreeWheelConfig` by adding the following line.
