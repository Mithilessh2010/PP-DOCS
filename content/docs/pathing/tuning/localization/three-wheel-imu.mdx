...
In `Constants.java`, add an instance of `ThreeWheelIMUConfig`. Make sure to replace the hardware map names
...
public static ThreeWheelIMUConfig localizerConfig = new ThreeWheelIMUConfig(
        c -> {
            ...
            c.leftEncoderName.set("leftFront");
...
Then, add `.threeWheelIMULocalizer` to `create`:
...
using `c.leftPodY.set()`, `c.rightPodY.set()` and `c.strafePodX.set()`.
...
To reverse an encoder, add one of the following to `ThreeWheelIMUConfig`:
...
Add the multiplier to your `ThreeWheelIMUConfig` by adding the following.
...
Lastly, add the multiplier to `ThreeWheelIMUConfig` by adding the following line.
...
```java title="Constants.java"
c.turnTicksToRadians.set(multiplier)
```
